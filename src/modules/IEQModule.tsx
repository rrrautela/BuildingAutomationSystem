...
export const IEQModule = () => {
  ...
  const zonesWithAlerts = zones.filter(
    (z) => z.co2 > 700 || z.pm25 > 25 || z.voc > 200 || Math.abs(z.temp - z.targetTemp) > 2
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
            ...
            <div className="flex flex-col items-center">
              <GaugeChart
                value={getAirQualityScore(selectedZone)}
                max={100}
                label="AQI"
                color="#22c55e"
              />
        ...
        <Card>
          ...
          {zonesWithAlerts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-gray-400 mt-1">
                All zones within acceptable IEQ parameters
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {zonesWithAlerts.map((zone) => {
                const issues = [];
                if (zone.co2 > 700) issues.push('High CO₂');
          ...
          <div className="mt-6 pt-6 border-t border-gray-800">
            ...
            <div className="space-y-3 text-sm">
              {[
                { label: 'CO₂', optimal: '< 600 ppm', warning: '600-700 ppm', critical: '> 700 ppm' },
                { label: 'PM2.5', optimal: '< 15 µg/m³', warning: '15-25 µg/m³', critical: '> 25 µg/m³' },
                ...
                <div key={standard.label} className="p-3 bg-gray-800/50 rounded-lg">
                  ...
                  <div className="flex gap-4 text-xs">
                    <span className="text-green-400">✓ {standard.optimal}</span>
                    <span className="text-yellow-400">⚠ {standard.warning}</span>
                    <span className="text-red-400">✕ {standard.critical}</span>
                  </div>
                </div>
              ))}
            </div>
